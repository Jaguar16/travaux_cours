...
(provide 
 ...
 pile-vide pile-vide? empiler depiler sommet couple proj1 proj2 fac)             ; les piles fonctionnelles
...
(define (couple a b)     ;; Creé un couple
  (lambda (c)
    (if c a b)))

(define (proj1 t)        ;; accede à la 1ère donnée d'un couple
  (t #t))

(define (proj2 t)        ;; accede à la 2ème donnée d'un couple
  (t #f))

(define (fac n)          ;; calcul la factorielle de n
  (if (= n 0)
      1
      (* n (fac (- n 1)))))
...
(pile-vide? P), (pile-vide), (empiler x P), (sommet P), (depiler P), (couple a b), (proj1 couple), (proj2 couple), (fac n)\n")
